...
import org.rabbit.service.email.models.TemplateRequestDTO;
...
/**
 * The handler class for parse email variables
 *
 * @author nine rabbit
 */
@Slf4j
@Service
public class DefaultParseEmailVariablesHandler {
    ...
    public DefaultParseEmailVariablesHandler(EmailTemplateService emailTemplateService) {
    ...
    private String parseSubject(EmailTemplate emailTemplate, Map<String, Object> variables) {
        TemplateRequestDTO templateRequestDTO = new TemplateRequestDTO();
        templateRequestDTO.setId(emailTemplate.getId().replaceAll(EmailTemplate.EMAIL_TEMPLATE_ID_PREFIX, "email_subject."));
        templateRequestDTO.setVariables(variables);
        try {
            return emailTemplateService.parse(templateRequestDTO);
        } catch (Exception ex) {
